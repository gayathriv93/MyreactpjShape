...
class Box extends Component {
  ...
  render() {
    return (
          ...
          <div className="box" style={{ backgroundColor :this.state.color , borderRadius : this.props.Shape === 's' ? '0%' : '50%' } }>
...
<button onClick = {()=> {
  this.setState ({Shape: this.state.Shape=== 'c'?'s':'c'} )
}}> Shape</button>
   
      {bcolors.map (c=> <Box Shape={this.state.Shape} key={Math.random()} color={c}></Box>)}
       </div>
